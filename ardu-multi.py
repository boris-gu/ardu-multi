...
from textual.widgets import Header, Footer, DataTable, TabbedContent, TabPane, Log
...
class ArduMultiApp(App):
    ...
    def compose(self) -> ComposeResult:
        table = DataTable(cursor_type = "none", zebra_stripes = True, classes="big_block")
        table.add_columns(("Mode", "mode_col"),
                          ...
                          ("Longitude", "lon_col"),
                          ("Height", "height_col"))
        tabs = TabbedContent(classes="big_block")
    ...
    def uart_rx(self):
        ...
        while True:
            ...
            log = self.query_one(f"#log_{1}", Log)
            self.call_from_thread(log.write_line, mav_s_text[i])
            i = (i+1) % len(mav_s_text)
            # #TODO: Избавиться от Sleep, он мешает!!!
            time.sleep(1)

    def new_statustext(self, id):
        new_tabs = self.query_one(TabbedContent)
        new_log = Log(id=f"log_{id}")
        pane = TabPane(f"Drone {id}", new_log)
